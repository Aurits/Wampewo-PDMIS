                        ...
                        <div class="row flex-grow">
                          <div class="">
                            ...
                            <div class="card-body mb-5">
                              ...
                              // Your PHP code to implement functionalities to manage citizen groups
                              // Replace the static data with dynamic data from the database

                              // Query example
                              $sql = "SELECT * FROM citizen_groups"; // Replace 'Citizen_Groups' with the actual table name
                              $result = mysqli_query($conn, $sql);

                              if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                  // Display citizen group details
                                  echo '<p>Group ID: ' . $row['ID'] . '</p>';
                                  echo '<p>Group Name: ' . $row['Name'] . '</p>';
                                  echo '<p>Description: ' . $row['Business_Description'] . '</p>';
                                  echo '<p>Funding Proposal: ' . $row['Funding_Proposal'] . '</p>';
                                  echo '<hr>';
                                }
                              } else {
